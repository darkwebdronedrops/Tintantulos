...
func _on_new_game():
	...
	GameState.new_game()
	# Play intro cinematic first, then Floor 1
	get_tree().change_scene_to_file("res://scenes/IntroCinematic.tscn")
...
func _on_play_tutorial():
	"""Force tutorial mode: reset first-run flags and play cinematic."""
	visible = false
	AudioManager.stop_music()
	GameState.is_first_run = true
	GameState.door_tutorial_completed = false
	GameState.save_game()
	print("[TitleScreen] Tutorial mode forced — playing cinematic then tutorial")
	get_tree().change_scene_to_file("res://scenes/IntroCinematic.tscn")
