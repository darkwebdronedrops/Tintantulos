...
func _build_floor():
	GameState.set_current_floor(10)
	print("[Floor10-Hex] current_floor set to 10")
	...
	if victory:
		room_cleared[current_room_id] = true
				...
				if post_combat_ui:
					...
					in_ui = true
					print("[Floor10-Hex] Post-combat UI shown, faction: %s" % defeated_faction)
...
func _setup_post_combat_ui():
	...
	if post_combat_scene:
		...
		print("[Floor10-Hex] PostCombatUI ready")
	else:
		push_warning("[Floor10-Hex] PostCombatUI scene not found!")
