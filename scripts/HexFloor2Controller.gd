...
func _setup_post_combat_ui():
	...
	if post_combat_scene:
		...
		print("[Floor2-Hex] PostCombatUI ready")
	else:
		push_warning("[Floor2-Hex] PostCombatUI scene not found!")
	...
	if victory:
		room_cleared[current_room_id] = true
				...
				if post_combat_ui:
					...
					in_ui = true
					print("[Floor2-Hex] Post-combat UI shown, faction: %s" % defeated_faction)
...
func _on_post_combat_closed():
	in_ui = false
	print("[Floor2-Hex] Post-combat closed, resuming")
